...
extra_dims <- str_extract(setdiff(ind_dims, base_dims), "\\d+")

extra_dim_names <- get_indicator_dimensions(indicator_id) %>% 
  filter(id %in% extra_dims) %>% 
  pull(name)

dim_ids <- if (length(extra_dims) > 0) paste(extra_dims, collapse = ", ") else ""
...
#   
#   cat(glue::glue("### QC Breakdown by {dim}\n\n"))
# 
...
#       color = .data[[dim]],
#       group = .data[[dim]],
#       text = glue("Group: {.data[[dim]]}<br>Value: {scales::comma(value_data)}")
#     )) +
#     geom_line(linewidth = 0.8) +
#     #geom_point() +
#     facet_wrap(~ dim_208_label, scales = "free_y") +
#     labs(
#       title = glue("Values by {dim}, over dim_29117_label"),
#       x = "dim_29117_label",
#       y = "Value",
#       color = dim
#     ) +

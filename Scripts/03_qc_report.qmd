...
output-file: "qc_report_5672.html"
---
...
```{r show new values by country, warning = FALSE, fig.height=14}
p <- comp %>% 
...
```{r display data comparison summary statistics}
summary_info <- comp %>%
...
```{r show absolute value differences}
p <- comp %>% 
...
if(!params$new_indicator) { # will throw error if no public data
  ggplotly(p, tooltip = "text") %>%
  layout(
    title = list(
      text = paste0(
...
```{r display percent difference table}
reactable(
  comp %>%
    mutate(
...
```{r display country z-score table}
reactable(
  comp %>%
    filter(flag_some_na == FALSE) %>%
...
```{r show country outlier time series}
#| warning: false
...
```{r show dimension series by country, warning = FALSE, message = FALSE, fig.height=16}
render_plot_for_dim <- function(dim) {
  ...
  htmltools::tagList(
    htmltools::h3("Dimension-Level Analysis"),
    ggplotly(p, tooltip = "text")
  )
}

if(!params$new_indicator){
  if (length(extra_dims) >= 1) print(render_plot_for_dim(extra_dims[1]))
  if (length(extra_dims) == 2) print(render_plot_for_dim(extra_dims[2]))
}
```

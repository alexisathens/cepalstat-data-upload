...
Custom scraping for World Heritage Convention -- this lives outside of the InforMEA website
```{python}
import pandas as pd
import time
...
row = mea[mea["id"] == "whc"].iloc[0]

url = "https://whc.unesco.org/en/statesparties" # define custom url outside of inforMEA website
treaty_id = row["id"]
...
try:
  ...
  heritage = pd.DataFrame(data, columns=headers)
  
  # Tag each row with its source treaty URL
  heritage["treaty_id"] = treaty_id
  
except Exception as e:
  print(f"Failed for {url}: {e}")

#all_parties_df = pd.concat(all_parties, ignore_index=True)
# Manually add after R cleaning to rest of treaties
```
...
# ok to be missing: heritage
```

## Scrape treaty metadata from web
***Note this doesn't need to be done annually - this data will stay consistent over time
```{python}
import pandas as pd
import time
...
# clean strings
parties %<>% 
  mutate(Party = str_trim(Party), Signature = str_trim(Signature)) %>% 
  mutate(across(where(is.character), ~ na_if(., "")))

# clean columns
parties %<>% 
  relocate(treaty_id, .before = "Party") %>% 
  select(-`Additional information`) %>% 
  rename(Force = Party_2) %>% 
  relocate(Status, .after = Party)

# clean dates
parties %<>% 
  mutate(across(c(Signature, Ratification, Force), ~ as.numeric(str_extract(., "\\d{4}"))))
```

Join MEA info
```{r}
parties %<>% 
  left_join(mea, by = c("treaty_id" = "id")) %>%
  select(treaty_id:titleEnglish) %>% 
  rename(mea_id = treaty_id, cs_id = keyword, # distinguish between the key in InforMEA and Cepalstat
         title = titleEnglish, title_full = officialNameEnglish) %>% 
  relocate(title_full, .after = title)
```

Clean and join Heritage treaty info
```{r}
parties <- read_csv(here("Data/Raw/informea/parties_raw.csv"))
heritage <- py$heritage

# Clean heritage file to match parties
heritage %<>% select(`State Party`:treaty_id) %>% as_tibble()

heritage %<>% 
  rename(Party = `State Party`, Instrument = `Type of instrument`) %>% 
  mutate(Instrument = case_when(
    Instrument == "R" ~ "Ratification",
    Instrument == "Ac" ~ "Acceptance",
    Instrument == "S" ~ "Succession",
    Instrument == "A" ~ "Accession",
    TRUE ~ Instrument
  )) %>% 
  mutate(across(c(Date), ~ as.numeric(str_extract(., "\\d{4}")))) %>% 
  rename(Ratification = Date, Status = Instrument)

# match parties
heritage %<>% 
  rename(mea_id = treaty_id) %>% 
  mutate(Signature = NA_integer_, Force = NA_integer_, cs_id = "Heritage",
         title = "World Heritage Convention", title_full = "Convention Concerning the Protection of the World Cultural and Natural Heritage") %>% 
  select(mea_id, Party, Status, Signature, Ratification, Force, cs_id, title, title_full)
...
```{r}
treaties %<>% 
  select(treaty_id, Keywords, Region, Parties = `Number of parties`, Summary, Force = `Entry into force`, Parent = `Parent treaty (first level)`) %>% 
  left_join(mea, by = c("treaty_id" = "id")) %>%
  rename(mea_id = treaty_id, cs_id = keyword, # distinguish between the key in InforMEA and Cepalstat
         title = titleEnglish, title_full = officialNameEnglish) %>%
  mutate(across(c(Force), ~ as.numeric(str_extract(., "\\d{4}")))) %>% 
  select(mea_id, cs_id, title, title_full, Keywords, Region, Parties, Force, Parent, Summary)
```

## Export flat files
```{r}
write_csv(parties, here("Data/Raw/informea/parties_raw.csv"))
write_csv(treaties, here("Data/Raw/informea/treaties_raw.csv"))
```

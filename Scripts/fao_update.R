# This script contains the migrated (indicator_spec / process_indicator) versions of the FAO
# indicators that are still old-style in fao.R, for a side-by-side comparison against that file.
# fao.R is left untouched -- once this is verified, these specs should replace the old-style
# blocks there (the forest indicators, 2036/2021/2530/2531, are already migrated in fao.R itself).
#
# This script assumes fao.R's "---- setup ----" / "---- download data ----" sections have already
# been run (through the "filter_forest" helper, before "## ---- indicator 2036 ----"), so that
# max_year_fao, use, clim, cover, crop, fert, pest, fish, aqua, and aquastat already exist in the
# environment. It does not redownload or redefine any of them.
...
# no ECLAC average (regional_fn = FALSE in old code) -- maintain_regional never adds a "Latin
# America and the Caribbean" row, so the old LAC-only footnote (6970) could never fire and is
# dropped here rather than carried over as dead code
...
define_source_4176 <- function(df, indicator_id) {
  df %>% mutate(source_id = 651) # general FAOSTAT source
}
...
spec_4176 <- indicator_spec(
  ...
  calculate_regional = calculate_regional_sum,
  define_source = define_source_4176
)
...
define_source_2022 <- function(df, indicator_id) {
  # Calculations made based on fertilizer consumption data and agriculture area data from
  # online statistical database (FAOSTAT) to Food and Agriculture Organization of the United Nations (FAO)
  df %>% mutate(source_id = 913)
}
...
spec_2022 <- indicator_spec(
  ...
  footnotes = lac_footnote,
  define_source = define_source_2022
)
...
define_source_3382 <- function(df, indicator_id) {
  # Calculations made based on pesticide consumption data and agriculture area data from
  # online statistical database (FAOSTAT) to Food and Agriculture Organization of the United Nations (FAO)
  df %>% mutate(source_id = 1827)
}
...
spec_3382 <- indicator_spec(
  ...
  calculate_regional = calculate_regional_intensity,
  footnotes = lac_footnote,
  define_source = define_source_3382
)
...
define_source_2039 <- function(df, indicator_id) {
  # Calculations made based on pesticide consumption data and agriculture area data from
  # online statistical database (FAOSTAT) to Food and Agriculture Organization of the United Nations (FAO)
  df %>% mutate(source_id = 1827)
}
...
spec_2039 <- indicator_spec(
  ...
  calculate_regional = calculate_regional_sum,
  footnotes = lac_footnote,
  define_source = define_source_2039
)


# FAO FISH INDICATORS -----

# Sub/total summarization shared by the fish/aquaculture production indicators (2019, 2020): the
# LAC total needs a "TOTAL"/"Total" row of its own, so it's simplest to build the per-country total
# here in transform_data and let the standard calculate_regional_sum sum it across countries too.
add_species_total <- function(data) {
  total <- data %>%
    group_by(Country, Years) %>%
    summarize(value = sum(value, na.rm = TRUE), .groups = "drop") %>%
...
footnotes_2019 <- list(
  "6545" = function(df) rep(TRUE, nrow(df)), # Incluye la captura en áreas marinas y en aguas continentales.

...
max_year <- 2025 # define most recent year with full data
...
# ---- indicator 4252 - Share of real exports of primary products according to non/renewable natural resources ----
...
dim_config_4252 <- tibble(
  data_col = c("Country", "Years", "Type"),
  dim_id = c("208", "29117", "78689"),
  pub_col = c("208_name", "29117_name", "78689_name")
)

filter_4252 <- function(data) {
  data %>% 
    select(Country, Date = `Designation date`, value = `Area (ha)`)
}

transform_4252 <- function(data) {
  data %>% 
    # expend year data since data source only lists the start year it was designated
    mutate(start_year = year(Date)) %>% 
    rowwise() %>%
...
result_4252 <- process_indicator(
  indicator_id = indicator_id,
  data = data_ramsar,
  dim_config = dim_config_4252,
  filter_fn = filter_4252,
  transform_fn = transform_4252,
  remove_lac = TRUE, # remove and recalculate LAC as simple sum
  footnotes_fn = footnotes_4252,
  diagnostics = TRUE,
  export = TRUE
)


# ---- indicator 4264 - Composition of exports of primary products according to non/renewable natural resources ----
...
dim_config_4264 <- tibble(
  data_col = c("Country", "Years"),
  dim_id = c("208", "29117"),
  pub_col = c("208_name", "29117_name")
)

# 1912 - Total exports of goods FOB, by product group: primary products according to SITC rev.1
prim_exports <- call.data(1912) %>% as_tibble() %>% select(Country, Years, prim = value)

# 1913 - Total exports of goods FOB, by product group: manufactures products according to SITC rev.1
man_exports <- call.data(1913) %>% as_tibble() %>% select(Country, Years, man = value)

# Calculate total exports (primary + manufacturing)
exports <- prim_exports %>% full_join(man_exports, by = c("Country", "Years")) %>% 
  rowwise() %>% mutate(total = prim + man) %>% ungroup()

# 812 - Price indices of leading export commodities yearly (est. for price index for primary products)
# -- note this series starts in year 2000; Base year 2018=100
prim_pi <- call.data(812) %>% as_tibble() %>% filter(Product == "Total") %>% select(Years, pi_prim = value)

# 882 - External trade Deflator (est. price index for total products)
# -- Index 2018 = 100
total_pi <- call.data(882) %>% as_tibble() %>% filter(Item == "Unit value indices for exports of goods and services") %>% select(Country, Years, pi_total = value)

exports %<>% 
  left_join(prim_pi, by = c("Years")) %>% 
  left_join(total_pi, by = c("Country", "Years"))

# Calculate real export values
exports %<>% 
  # filter(Country == "Latin America and the Caribbean" & Years == 2000) %>% # cross-check Alberto calculations
  mutate(prim_real = prim / pi_prim * 100) %>% 
  mutate(total_real = total / pi_total * 100)


exports

# data_nr %>% filter(complete.cases(full))

data_nr <- exports %>% 
  filter(complete.cases(exports)) %>% 
  filter(Country == "Latin America and the Caribbean")




filter_4264 <- function(data) {
  data %>% 
    select(Country, Date = `Designation date`, value = `Area (ha)`)
}

transform_4264 <- function(data) {
  data %>% 
    # expend year data since data source only lists the start year it was designated
    mutate(start_year = year(Date)) %>% 
    rowwise() %>%
...
result_4264 <- process_indicator(
  indicator_id = indicator_id,
  data = data_ramsar,
  dim_config = dim_config_4264,
  filter_fn = filter_4264,
  transform_fn = transform_4264,
  remove_lac = TRUE, # remove and recalculate LAC as simple sum
  footnotes_fn = footnotes_4264,
  diagnostics = TRUE,
  export = TRUE
)
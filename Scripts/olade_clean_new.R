# This script processes OLADE energy indicators using the indicator_spec/process_indicator architecture
...
# Shared no-op filter for indicators whose old filter_fn only did the now-redundant Years <= max_year
# check (assert_data_reqs() already enforces this).
filter_none <- function(data) data

# Default source for the indicators newly cloned from OLADE (5672, 5730)
define_source_olade <- function(df, indicator_id) {
  df %>% mutate(source_id = 714) # OLADE - Economic Energy Information System
}

# Default source for the ECLAC-calculated variation-rate indicators (4183, 4184)
define_source_olade_calc <- function(df, indicator_id) {
  df %>% mutate(source_id = 10685) # Calculations made by ECLAC based on the economic and energy information system of OLADE
}
...
spec_5672 <- indicator_spec(
  ...
  define_source = define_source_olade,
...
spec_4183 <- indicator_spec(
  ...
  calculate_regional = maintain_regional, # keep source LAC data from OLADE
  define_source = define_source_olade_calc
)
